...
// D12/§7.5: QR is a UI convenience on the same token already generated by
// the going-RSVP trigger, not a new send. Best-effort — a fetch failure just
// means the email goes out without the QR, never blocks the confirmation.
async function fetchCheckinToken(
  admin: AdminClient,
  eventId: string,
  userId: string
): Promise<string | null> {
  const { data, error } = await admin
    ...
    .eq("event_id", eventId)
    .eq("user_id", userId)
    .maybeSingle<{ checkin_token: string | null }>();
  if (error) {
    log.warn("events email: fetch checkin token failed", {
      action: "event_email_fetch_checkin_token",
      event_id: eventId,
      user_id: userId,
      error_message: error.message,
    });
  ...
  return data?.checkin_token ?? null;
}
...
export async function sendEventRsvpConfirmation({
  ...
  const [event, profile, checkinToken] = await Promise.all([
    fetchEvent(admin, eventId),
    fetchProfile(admin, userId),
    fetchCheckinToken(admin, eventId, userId),
  ]);
  ...
  // Best-effort: a QR generation failure shouldn't block the confirmation
  // send, the email still works fine without it (check in on the site).
  // Sent as a real inline attachment (cid:), not a data: URI — Gmail, this
  // app's primary client, strips base64 data: images from HTML entirely.
  const checkinQrBuffer = checkinToken
    ? await QRCode.toBuffer(checkinToken, { margin: 1, width: 200 }).catch(
        () => null
      )
    : null;
  const checkinQrCid = checkinQrBuffer ? "checkin-qr" : null;
  ...
  const sendResult = await sendEmail({
    ...
    attachments: checkinQrBuffer
      ? [
          {
            filename: "checkin-qr.png",
            content: checkinQrBuffer,
            contentType: "image/png",
            contentId: checkinQrCid!,
          },
        ]
      : undefined,
  });
