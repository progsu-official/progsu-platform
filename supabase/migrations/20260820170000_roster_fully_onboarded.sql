-- Day-of check-in needs to show staff, at a glance, whether each attendee has
-- finished profile onboarding (docs/14-low-friction-signup: RSVP no longer
-- requires it up front, so many "going" attendees will still be mid-funnel).
-- Reuse the canonical public.is_fully_onboarded() helper (CLAUDE.md rule #5)
-- rather than re-deriving the gate here — one extra column on the existing
-- roster RPC, no new round trip.

-- Return type is changing (new OUT column) — must drop before recreating.
...
begin
  ...
  select
    ...
    ei.invited_by,
    ei.invited_at,
    public.is_fully_onboarded(u.user_id)
  from roster_users u
