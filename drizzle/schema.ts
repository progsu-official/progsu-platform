...
export const events = pgTable("events", {
	...
	checkInCodeExpiresAt: timestamp("check_in_code_expires_at", { withTimezone: true, mode: 'string' }),
...
}).with({"securityInvoker":true}).as(sql`SELECT p.id AS user_id, pvs.profile_slug, COALESCE(NULLIF(TRIM(BOTH FROM p.preferred_name), ''::text), p.first_name) AS display_name, p.avatar_url, p.school, p.class_standing, p.grad_term, p.grad_year, p.interested_roles, pvs.share_attended_events, pvs.last_discoverability_change_at AS visible_since FROM profile_visibility_settings pvs JOIN profiles p ON p.id = pvs.user_id WHERE pvs.discoverable = true AND p.is_archived = false`);