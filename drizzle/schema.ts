...
END`),
	...
	index("profiles_phone_e164_idx").using("btree", table.phoneE164.asc().nullsLast().op("text_ops")).where(sql`(phone_e164 IS NOT NULL)`),
	...
	unique("profiles_checkin_code_key").on(table.checkinCode),
...
export const legacyMembers = pgTable("legacy_members", {
	...
	smsConsentAt: timestamp("sms_consent_at", { withTimezone: true, mode: 'string' }),
	smsConsentCopy: text("sms_consent_copy"),
	answeredAt: timestamp("answered_at", { withTimezone: true, mode: 'string' }),
}, (table) => [
	index("legacy_members_answered_idx").using("btree", table.answeredAt.desc().nullsFirst().op("timestamptz_ops")).where(sql`(answered_at IS NOT NULL)`),
	...
	index("legacy_members_phone_e164_idx").using("btree", table.phoneE164.asc().nullsLast().op("text_ops")).where(sql`(phone_e164 IS NOT NULL)`),
...
export const events = pgTable("events", {
	...
	sendSmsReminder: boolean("send_sms_reminder").default(true).notNull(),
	smsReminderSentAt: timestamp("sms_reminder_sent_at", { withTimezone: true, mode: 'string' }),
}, (table) => [
	...
	index("events_sms_reminder_due_idx").using("btree", table.startsAt.asc().nullsLast().op("timestamptz_ops")).where(sql`((status = 'published'::event_status_t) AND (send_sms_reminder = true) AND (sms_reminder_sent_at IS NULL))`),
	index("events_status_starts_at_idx").using("btree", table.status.asc().nullsLast().op("enum_ops"), table.startsAt.desc().nullsFirst().op("enum_ops")),
	foreignKey({
			columns: [table.createdBy],
			foreignColumns: [profiles.id],
...
export const selfEventHistory = pgView("self_event_history", {	eventId: uuid("event_id"),
	...
	attended: boolean(),
	coverImagePath: text("cover_image_path"),
}).as(sql`SELECT e.id AS event_id, e.slug, e.title, e.starts_at, e.ends_at, e.status, e.visibility, e.location_text, r.status AS rsvp_status, r.status_changed_at AS rsvp_changed_at, r.waitlisted_at, a.checked_in_at, a.method AS attendance_method, a.checked_in_at IS NOT NULL AS attended, e.cover_image_path FROM events e LEFT JOIN event_rsvps r ON r.event_id = e.id AND r.user_id = auth.uid() LEFT JOIN event_attendances a ON a.event_id = e.id AND a.user_id = auth.uid() WHERE r.user_id IS NOT NULL OR a.user_id IS NOT NULL UNION ALL SELECT e.id AS event_id, e.slug, e.title, e.starts_at, e.ends_at, e.status, e.visibility, e.location_text, NULL::rsvp_status_t AS rsvp_status, NULL::timestamp with time zone AS rsvp_changed_at, NULL::timestamp with time zone AS waitlisted_at, h.checked_in_at, NULL::attendance_method_t AS attendance_method, true AS attended, e.cover_image_path FROM historical_event_attendances h JOIN legacy_members lm ON lm.id = h.legacy_member_id JOIN events e ON e.id = h.event_id WHERE lm.claimed_profile_id = auth.uid() AND lower(h.approval_status) = 'approved'::text`);
