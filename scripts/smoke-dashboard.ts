...
async function main() {
  ...
  try {
    ...
    // 1. Fully onboarded user hits /dashboard → 200.
    const r1 = await fetch("http://localhost:3000/dashboard", {
      redirect: "manual",
      headers: { cookie },
    });
    ...
    console.log(`  ✓ fully onboarded → /dashboard 200 with profile + resume`);
    ...
    const r2 = await fetch("http://localhost:3000/dashboard", {
      redirect: "manual",
      headers: { cookie },
    });
    if (r2.status !== 307 || !r2.headers.get("location")?.includes("/onboarding/consent"))
      throw new Error(`r2 ${r2.status} ${r2.headers.get("location")}`);
    console.log(`  ✓ revoked age consent → /dashboard bounces to /onboarding/consent`);
    ...
    // 3. /dashboard/settings renders (consents + profile + resume sections).
    const r3 = await fetch("http://localhost:3000/dashboard/settings", {
      redirect: "manual",
      headers: { cookie },
    });
    ...
    console.log(`  ✓ /dashboard/settings renders all sections`);
