...
- Member's event page (`/events/[slug]`) renders the token as a QR on a personal ticket whenever RSVP = `going` (2026-08-18: was window-gated; relaxed so the ticket doubles as a keepsake — redemption stays admin-only server-side, so early visibility grants nothing).
