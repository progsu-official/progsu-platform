...
| D5 | Check-in code behavior | One active self-check-in code per event window. Stable by default during the window, but admins can rotate it manually. |
...
10. No QR scanner native app or offline kiosk mode.
...
- `event_visibility_t`
- `rsvp_status_t`
- `attendance_method_t` (`admin_click`, `self_code`)
