  ...
  const [{ data: profile }, { data: currentResume }, { data: domains }, { data: consents }, { data: versions }] =
    await Promise.all([
      supabase
        .from("profiles")
        .select(
          "first_name, last_name, preferred_name, school, major, minor, class_standing, grad_year, grad_term, interested_roles, linkedin_url, github_url, portfolio_url, phone_number, google_email, student_email, student_email_verified, student_email_verified_at, pending_domain_name, discord_username, discord_user_id"
        )
        .eq("id", user.id)
        .single(),
      supabase
